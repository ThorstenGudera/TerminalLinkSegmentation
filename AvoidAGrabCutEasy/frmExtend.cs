...
namespace AvoidAGrabCutEasy
{
    public partial class frmExtend : Form
    {
        ...
        //private DesignerControl.DesignerCtrl? _desCtrl;
        ...
        public frmExtend(Bitmap bmp, Bitmap bmpOrig, string basePathAddition/*, DesignerControl.DesignerCtrl? desCtrl*/)
        {
            InitializeComponent();

            CachePathAddition = basePathAddition;
            //this._desCtrl = desCtrl;
        ...
        private void btnExtendOrShrink_Click(object sender, EventArgs e)
        {
            ...
            if (!this.backgroundWorker1.IsBusy && this._bmpOrig != null && this.helplineRulerCtrl1.Bmp != null)
            {
                ...
                string? lj = this.cmbLineJoin.SelectedItem?.ToString();
                float ml = (int)this.numMiterLimit.Value;

                double eps = (double)this.numEpsilon.Value;
                bool remCol = this.cbRemCol.Checked;
                bool apprxL = this.cbApprxL.Checked;
                bool closeFigures = this.cbCF.Checked;
                ...
                if (lj != null)
                {
                    ...
                    this.backgroundWorker1.RunWorkerAsync(new object[] { bmp, bOrig, width, opaque, morph, diskshaped, once, lj, ml,
                        eps, remCol, apprxL, closeFigures });
                }
            }
        }
        ...
        private void frmExtend_Load(object sender, EventArgs e)
        {
            ...
            string[] l = System.Enum.GetNames(typeof(LineJoin));
            foreach (string s in l)
                this.cmbLineJoin.Items.Add(s);

            this.cmbLineJoin.SelectedIndex = 2;

            numExtendOrShrink_ValueChanged(this.numExtendOrShrink, new EventArgs());
        }
        ...
        private void backgroundWorker1_DoWork(object? sender, DoWorkEventArgs e)
        {
            if (e.Argument != null)
            {
                ...
                bool once = (bool)o[6];
                string lj = (string)o[7];
                float ml = (float)o[8];

                double eps = (double)o[9];
                bool remCol = (bool)o[10];
                bool apprxL = (bool)o[11];
                bool closeFigures = (bool)o[12];
                ...
                else
                {
                    if (width < 0)
                        ...
                        bResult = ExtendOutlineEx(bmp, bOrig, width, true, true, lj, ml, eps, remCol, apprxL, closeFigures);
                }
        ...
        private Bitmap ExtendOutlineEx(Bitmap bmp, Bitmap bOrig, int outerW, bool dontFill, bool drawPath, string lineJoin,
            float miterLimit, double eps = 0.7171, bool remCol = true, bool apprxL = false, bool closeFigures = true)
        {
            Bitmap bOut = new Bitmap(bmp.Width, bmp.Height);
            ...
            if (lInner?.Count > 0 && bOrig != null)
            {
                ...
                for (int i = 0; i < lInner.Count; i++)
                {
                    ...
                    if (remCol || apprxL)
                    {
                        ChainFinder cf = new ChainFinder();
                        if (remCol)
                            pts = cf.RemoveColinearity(pts, true);
                        if (apprxL)
                            pts = cf.ApproximateLines(pts, eps);
                    }
                        ...
                        using (GraphicsPath gp = new GraphicsPath())
                        {
                            ...
                            if (gp.PointCount > 0)
                            {
                                using (Graphics gx = Graphics.FromImage(bOut))
                                {
                                    ...
                                    using (Pen pen = new Pen(tb, 1))
                                    {
                                        pen.LineJoin = (LineJoin)System.Enum.Parse(typeof(LineJoin), lineJoin);
                                        ...
                                        if (drawPath && outerW > 0)
                                        {
                                            try
                                            {
                                                using (Pen pen2 = new Pen(tb, outerW))
                                                {
                                                    ...
                                                    pen2.LineJoin = (LineJoin)System.Enum.Parse(typeof(LineJoin), lineJoin);
                                                    pen2.MiterLimit = miterLimit;
                                                    gp.Widen(pen2);
                                                    gx.DrawPath(pen2, gp);
                                                }
                                            }
        ...
        //{
        //    List<DesignerControl.ShapeBase>? bmpList = this._desCtrl?.FList.Where((a) => a.GetType().Equals(typeof(DesignerControl.BitmapShape))).ToList();

        //    if (bmpList != null && bmpList.Count > 0)
        //    {
        //        using (frmSelectBitmapShape frm = new DesignerControl.frmSelectBitmapShape(this._desCtrl, bmpList, DesignerControl.PreviewListBox.PrevItemHeight / (float)(bmpList[0].ThumbnailImage.Size.Height), true))
        //        {
        //            frm.CheckBox1.Visible = true;
        //            if (frm.ShowDialog() == DialogResult.OK)
        //            {
        //                if (frm.PreviewListBox1.SelectedIndex > -1 && frm.PreviewListBox1.SelectedIndex < bmpList.Count)
        //                {
        //                    Bitmap? bmp = null;
        //                    try
        //                    {
        //                        if (this._desCtrl != null)
        //                            bmp = (Bitmap)((DesignerControl.BitmapShape)(this._desCtrl.FList.Where((a) => a.ID == bmpList[frm.PreviewListBox1.SelectedIndex].ID).ToArray()[0])).Bmp.Clone();
        //                    }
        //                    catch
        //                    {
        //                    }
        ...
        public void HighlightLoadPic()
        {
            btnLoadBasePic.Font = new Font(btnLoadBasePic.Font.FontFamily, btnLoadBasePic.Font.Size, FontStyle.Bold);
            btnGetBmp.Font = new Font(btnGetBmp.Font.FontFamily, btnGetBmp.Font.Size, FontStyle.Bold);
            label16.ForeColor = Color.Red;
        }

        private void cbMorphological_CheckedChanged(object sender, EventArgs e)
        {
            this.cbDiskShaped.Enabled = cbMorphological.Checked;
            this.cmbLineJoin.Enabled = this.groupBox3.Enabled = this.numMiterLimit.Enabled = !cbMorphological.Checked;

            if (this.cmbLineJoin.SelectedIndex == 0)
                this.numMiterLimit.Enabled = true;
            else
                this.numMiterLimit.Enabled = false;
        }

        private void cmbLineJoin_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!this.cbMorphological.Checked && this.numExtendOrShrink.Value > 0)
            {
                this.btnUndo.PerformClick();
                this.btnExtendOrShrink.PerformClick();
            }
        ...
        private void numMiterLimit_ValueChanged(object sender, EventArgs e)
        {
            if (!this.cbMorphological.Checked && (this.numExtendOrShrink.Value > 0 || this.numExtendOrShrink.Value > 3))
            {
                this.btnUndo.PerformClick();
                this.btnExtendOrShrink.PerformClick();
            }
        ...
        private void cbRemCol_CheckedChanged(object sender, EventArgs e)
        {
            this.numMiterLimit_ValueChanged(this.numMiterLimit, new EventArgs());
        }

        private void numEpsilon_ValueChanged(object sender, EventArgs e)
        {
            this.numMiterLimit_ValueChanged(this.numMiterLimit, new EventArgs());
        }

        private void numExtendOrShrink_ValueChanged(object sender, EventArgs e)
        {
            if (this.numExtendOrShrink.Value < 1)
                this.groupBox3.Enabled = this.numMiterLimit.Enabled = this.cmbLineJoin.Enabled = false;
            else
                this.groupBox3.Enabled = this.numMiterLimit.Enabled = this.cmbLineJoin.Enabled = true;
        }
    }
}
