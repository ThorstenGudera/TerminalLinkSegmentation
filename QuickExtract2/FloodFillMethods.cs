...
namespace QuickExtract2
{
    public class FloodFillMethods
    {
                ...
                if (smooth)
                {
                    if (AvailMem.AvailMem.checkAvailRam(bmp.Width * bmp.Height * 8L))
                    {
                        ...
                        try
                        {
                            ...
                            byte[] p2 = new byte[(bmD.Stride * bmD.Height)];
        ...
        public static void writeData2(Bitmap bmp, Dictionary<int, int> Pixellist, Dictionary<int, double> Pixellist2, Color z, double minVal, bool smooth, Bitmap? bmpCopy, bool compAlpha, double epsilon, bool useCurves, bool useClosedCurve)
        {
            if (AvailMem.AvailMem.checkAvailRam(bmp.Width * bmp.Height * 4L))
            {
                if (smooth)
