...
Public Class GradNormalBVectorAlgorithm
    ...
    Public Function GetValue(x As Integer, y As Integer, x2 As Integer, y2 As Integer, plane As Integer,
        ...
        Dim value As Double = 0
        If j < 2 Then
            value = (CDbl(pX(x * 4 + y * scanline + plane)) - CDbl(pX(x2 * 4 + y2 * scanline + plane)))
        Else
            value = (CDbl(pY(x * 4 + y * scanline + plane)) - CDbl(pY(x2 * 4 + y2 * scanline + plane)))
        End If
    ...
    Public Function GetValue(x As Integer, y As Integer, x2 As Integer, y2 As Integer, plane As Integer,
                             pX() As Byte, pY() As Byte, pB() As Byte, scanline As Integer, scanlineL As Integer,
                             rc As Rectangle, ptC As PointF, j As Integer) As Double Implements IBVectorComputingAlgorithm.GetValue
        Dim value As Double = 0
        If j < 2 Then
            value = (CDbl(pX(x * 4 + y * scanline + plane) - 127) - CDbl(pX(x2 * 4 + y2 * scanline + plane) - 127))
        Else
