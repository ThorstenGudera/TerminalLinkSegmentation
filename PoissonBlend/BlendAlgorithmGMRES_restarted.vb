...
Public Class BlendAlgorithmGMRES_restarted
    ...
    Public Sub Apply(rc As Rectangle) Implements IBlendAlgorithm.Apply
        ...
        Try
            If AvailMem.AvailMem.checkAvailRam(_blendParameters.UpperImg.Width * _blendParameters.UpperImg.Height * 4L + _blendParameters.LowerImg.Width * _blendParameters.LowerImg.Height * 4L) Then
                ...
                'V and Wl for all inneriterations and all planes + other arrays (5 * b.Length * doubleSize * 3-planes)
                If AvailMem.AvailMem.checkAvailRam((innerIterations * pixels.Count * 64L) + (pixels.Count * 360L)) Then
                    ...
                    Parallel.For(0, 3, Sub(plane, loopState)
                                           ...
                                           Dim x_Cur(b.Length - 1) As Double
                                           Dim r_Cur(b.Length - 1) As Double

                                           If preRelax Then 'evtl. show progres
                                               RaiseEvent ShowInfo(Me, " PreRelaxing")
                                               Dim prg As Integer = _blendParameters.PE.CurrentProgress
                                               x_Cur = GaussSeidelForPoissonEq_Div(b, n, r, PRE_OP_GSSOR, True, 0.01, _blendParameters.PE, w)
                                               GetResidual(r_Cur, x_Cur, b, n, r)
                                               _blendParameters.PE.CurrentProgress = prg
                                           Else
                                               b.CopyTo(r_Cur, 0)
                                           End If

                                           r_Cur = SSOR_Apply(r_Cur, w, n, r)
                                           Dim eVal As Double = maxError * 10
                                           ...
                                           For z2 = 1 To restart
                                               ...
                                               Dim V(innerIterations)() As Double
                                               Dim rNorm As Double = GetNorm(r_Cur)

                                               If rNorm = 0 Then
                                                   rNorm = 1
                                               End If

                                               Dim t(b.Length - 1) As Double
                                               V(0) = t
                                               For i As Integer = 0 To b.Length - 1
                                                   V(0)(i) = r_Cur(i) / rNorm
                                               Next

                                               'V(0) = SSOR_Apply(V(0), 1.5, n, r)

                                               Dim H(innerIterations, innerIterations - 1) As Double
                                               Dim wl(innerIterations - 1)() As Double
                                               Dim cs(innerIterations) As Double
                                               Dim sn(innerIterations) As Double
                                               Dim y(innerIterations) As Double
                                               Dim gamma(innerIterations) As Double
                                               gamma(0) = rNorm

                                               Dim cnt2 As Integer = 0

                                               For j As Integer = 0 To innerIterations - 1
                                                   If Me.CancellationPending Then
                                                       Exit For
                                                   End If

                                                   'Arnoldi
                                                   Dim q As Double() = MultiplyA(V(j), n, r)
                                                   q = SSOR_Apply(q, w, n, r)
                                                   ...
                                                   Dim beta As Double = Math.Sqrt(H(j, j) * H(j, j) + H(j + 1, j) * H(j + 1, j))
                                                   If H(j, j) = 0 Then
                                                       sn(j + 1) = 1
                                                       cs(j + 1) = 0
                                                   End If
                                                   ...
                                                   SyncLock Me._lockObject
                                                       If _blendParameters.PE IsNot Nothing Then
                                                           If _blendParameters.PE.ImgWidthHeight < Int32.MaxValue Then
                                                               _blendParameters.PE.CurrentProgress += 10 \ (innerIterations \ 5)
                                                           End If
                                                           Try
                                                               If CInt(_blendParameters.PE.CurrentProgress) Mod _blendParameters.PE.PrgInterval = 0 Then
                                                   ...
                                                   If Math.Abs(gamma(j + 1)) / rNorm >= (maxError / rNorm) AndAlso j < innerIterations - 1 Then
                                                       V(j + 1) = Pr(wl(j), 1.0 / H(j + 1, j))
                                                       H(j + 1, j) = 0
                                                       'V(j + 1) = SSOR_Apply(V(j + 1), 0.7, n, r)
                                                   Else
                                                       ...
                                                       For i As Integer = 0 To j
                                                           ...
                                                           Dim z() As Double = Pr(V(i), y(i))
                                                           For l As Integer = 0 To b.Length - 1
                                                               rz(l) += z(l)
                                                           Next
                                                       Next
                                               ...
                                               If Math.Abs(eVal) / rNorm < (maxError / rNorm) OrElse z2 = restart - 1 Then
                                                   cnt = z2 * innerIterations + cnt2 + 1
                                                   Exit For
                                               Else
                                                   GetResidual(r_Cur, x_Cur, b, n, r)
                                                   r_Cur = SSOR_Apply(r_Cur, w, n, r)
                                                   RaiseEvent ShowInfo(Me, "### --- ### Info: Restart: " & (z2 + 1).ToString() & " on plane " & plane.ToString() & ". Max Error: " & Math.Abs(eVal).ToString())
                                                   cnt = (z2 - 1) * innerIterations + cnt2 + 1
                                               End If
                                           Next

                                           'make the results re-usable for further iterations
                                           _blendParameters.Results(plane) = x_Cur
                    ...
                    'Next
                Else
                    RaiseEvent ShowInfo(Me, " Not enough Memory")
                    Throw New Exception("Not enough Memory")
                End If

                OnShowProgress(_blendParameters.PE.ImgWidthHeight)
                RaiseEvent ShowInfo(Me, " Average amount of iterations: " & (cnt4).ToString())
                ...
                If AvailMem.AvailMem.checkAvailRam(uI.Width * uI.Height * 4L + bParams.LowerImg.Width * bParams.LowerImg.Height * 4L) Then
                    ...
                    If AvailMem.AvailMem.checkAvailRam((innerIterations * pixels.Count * 64L) + (pixels.Count * 360L)) Then
                        ...
                        Parallel.For(0, 3, Sub(plane, loopState)
                                               ...
                                               'make the results re-usable for further iterations
                                               bParams.Results(plane) = x_Cur
                                               bParams.DiagonalElements(plane) = n
                                               bParams.OtherElements(plane) = r

                                               bParams.BVector(plane) = b
    ...
    Private Sub SetMask(bmp As Bitmap, upperImg As Bitmap, distance As Integer, satInc As Single)
        If Not AvailMem.AvailMem.checkAvailRam(bmp.Width * bmp.Height * 15L) Then
            Return
        End If
        ...
        RaiseEvent ShowInfo(Me, "Setting Mask")
        Dim bmData As BitmapData = bmp.LockBits(New Rectangle(0, 0, bmp.Width, bmp.Height), ImageLockMode.ReadWrite, PixelFormat.Format32bppArgb)
        Dim bmSrc As BitmapData = upperImg.LockBits(New Rectangle(0, 0, upperImg.Width, upperImg.Height), ImageLockMode.ReadWrite, PixelFormat.Format32bppArgb)
        ...
        RaiseEvent ShowInfo(Me, "Increasing Saturation")
        Parallel.[For](0, h, Sub(y)
                                 Dim pos As Integer = y * stride
                                 For x As Integer = 0 To w - 1
                                     Dim d As Integer = distanceArray(x, y)

                                     Dim red As Byte = p(pos + 2)
                                     Dim green As Byte = p(pos + 1)
                                     Dim blue As Byte = p(pos)

                                     Dim hsl As HSLData = RGBtoHSL(red, green, blue)
                                     Dim v As Single = 0

                                     If d < distance Then
                                         v = CSng(CDbl(d) / 65) * satInc
                                     Else
                                         v = satInc
                                     End If

                                     Dim val_saturation As Single = Math.Max(Math.Min(hsl.Saturation + v, 1.0F), 0F)

                                     Dim nPixel As PixelData = HSLtoRGB(hsl.Hue, val_saturation, hsl.Luminance)
                                     p(pos) = nPixel.blue
                                     p(pos + 1) = nPixel.green
                                     p(pos + 2) = nPixel.red
    ...
    Private Sub ApplyRot(h(,) As Double, cs() As Double, sn() As Double, i As Integer, j As Integer)
        Dim temp As Double = cs(i + 1) * h(i, j) + sn(i + 1) * h(i + 1, j)
        h(i + 1, j) = -sn(i + 1) * h(i, j) + cs(i + 1) * h(i + 1, j)
        h(i, j) = temp
    End Sub

    Private Function ComputeWj(q() As Double, H(,) As Double, V()() As Double, j As Integer) As Double()
        Dim w(q.Length - 1) As Double
        Dim rz(q.Length - 1) As Double

        For i As Integer = 0 To j
            Dim z() As Double = Pr(V(i), H(i, j))
            For l As Integer = 0 To q.Length - 1
                rz(l) += z(l)
            Next
        Next
    ...
    Private Function MultiplyA(y() As Double, n() As Integer, r As Integer()()) As Double()
        Dim z(y.Length - 1) As Double
    ...
    Private Function Pr(v() As Double, d As Double) As Double()
        Dim l(v.Length - 1) As Double
    ...
    Private Function ScPr(v1() As Double, v2() As Double) As Double
    ...
    'http://www4.ncsu.edu/eos/users/w/white/www/white/ma580/chap6.4.PDF
    'I: x_i+1 = x_i + [(1/w*D-U)^-1 * (2-w)/w * D * (1/w*D-L)^-1] * (b-A*x_i)
    'II: C_ssor = w * (2-w) * (D-w*U)^-1 * D * (D-w)^-1 * L
    'III: (b - A*x) = r
    'Split II (or I) into three loops, consider III:
    'r is current Residual, rHat the new, updated one
    '1.: [(w*L) / D] * r = rHat
    '2.: [((2-w) / w) * D] * rHat [= rHat] '//-> can be put into third loop
    '3.: [(w*U) / D] * rHat [= rHat]
    'Note, for BiCGSTAB the omega (w) shouldnt be as strong as usual. Else it might be, that the error doesnt decrease.
    'Note, I use this symmetric preconditioner, because its easy to apply, maybe an Incomplete Cholesky Preconditioner might give better results for this problem...
    Private Function SSOR_Apply(r As Double(), w As Double, n() As Integer, rr()() As Integer) As Double()
        Dim rHat As Double() = (Enumerable.Repeat(0.0, r.Length).ToArray())

        Dim l As Integer = r.Length

        'read new values that are left or up (= previously computed inside this loop)
        For i As Integer = 0 To l - 1 'rows
            ...
            rHat(i) = r(i)
            Dim d As Double = w * (r(i) + sigma) / n(i)

            If Not Double.IsInfinity(d) AndAlso Not Double.IsNaN(d) Then
                rHat(i) = d
            End If
        Next

        'combined with next loop
        'For i As Integer = 0 To rHat.Length - 1
        '    rHat(i) *= ((2 - w) / w) * 4.0
        'Next

        'read new values that are right or down (= previously computed inside this loop -> we move over the data backwards)
        For i As Integer = l - 1 To 0 Step -1 'rows
            rHat(i) *= ((2 - w) / w) * 4.0
            ...
            Dim d As Double = w * (rHat(i) + sigma) / n(i)
    ...
    Private Sub GetResidual(r_Cur() As Double, x_Cur() As Double, bVector() As Double, n() As Integer, r()() As Integer)
        If r_Cur IsNot Nothing Then
            For i As Integer = 0 To r_Cur.Length - 1
                Dim v As Double = 0
                ...
                If r IsNot Nothing Then
                    ...
                    r_Cur(i) = bVector(i) - ((n(i) * x_Cur(i)) - v)
                End If
            Next
        End If
    End Sub

    Private Function GetNorm(r As Double()) As Double
        Dim e As Double = 0.0
    ...
    Private Function GaussSeidelForPoissonEq_Div(b As Double(), n As Integer(), r As Integer()(), maxIterations As Integer, autoStop As Boolean,
                                            autoStopTolerance As Double, pe As ProgressEventArgs, w As Double) As Double()
        ...
        For p As Integer = 0 To maxIterations - 1
            ...
            If autoStop Then
                If CheckResult(xVector, xVectorOld, autoStopTolerance) Then
                    '#If DEBUG Then
    ...
    Public Sub ApplyTile(upperImg As Bitmap, rc As Rectangle, rr As Rectangle, glBounds As List(Of ChainCode), xx As Integer, yy As Integer) Implements IBlendAlgorithm.ApplyTile
        ...
        Dim innerIterations As Integer = Me._blendParameters.GMRESParams.MaxInnerIterationsGMRES
        Dim maxError As Double = Me._blendParameters.GMRESParams.DesiredMaxLinearError
        Dim preRelax As Boolean = Me._blendParameters.GMRESParams.PreRelax
        Dim restart As Integer = Me._blendParameters.GMRESParams.MaxRestartAmountGMRES
    ...
    Public Sub ApplyDForAlpha(rc As Rectangle) Implements IBlendAlgorithm.ApplyDForAlpha
        'GMRES_r
        ...
        Try
            If AvailMem.AvailMem.checkAvailRam(_blendParameters.UpperImg.Width * _blendParameters.UpperImg.Height * 4L + _blendParameters.LowerImg.Width * _blendParameters.LowerImg.Height * 4L) Then
                ...
                'V and Wl for all inneriterations and all planes + other arrays (5 * b.Length * doubleSize * 3-planes)
                If AvailMem.AvailMem.checkAvailRam((innerIterations * pixels.Count * 64L) + (pixels.Count * 120L)) Then
                    'For plane As Integer = 0 To 2
                    Parallel.For(0, 3, Sub(plane, loopState)
                                           Dim cnt As Integer = 0
